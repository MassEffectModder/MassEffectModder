...
/*
 * Ported hash buckets algorithm from AutoTOC ASI code:
 * https://github.com/SirCxyrtyx/AutoTOC-asi/blob/master/AutoTOC-asi/dllmain.cpp
 */
...
static quint32 hashFilename(const void *data, quint32 length)
{
    quint32 hash = 0;
    auto *buffer = static_cast<const quint8 *>(data);

    while (length-- != 0)
    {
        hash = crcTable[(hash ^ *buffer++) & 0xFF] ^ (hash >> 8);
...
{
    QStringList files;
    QString gamePath = g_GameData->GamePath();
    ...
    for (int f = 0; f < files.count(); f++)
    {
        ...
        QString filenameToHash = BaseName(files[f]).toUpper();
        file.hashFilename = hashFilename(filenameToHash.toStdString().c_str(), filenameToHash.length());
...
void TOCBinFile::CreateTocBinFile(QString &path, const QVector<FileEntry>& filesList)
{
    if (QFile(path).exists())
        QFile::remove(path);

    quint32 tableSize = filesList.size();
    quint32 minTableSize = tableSize / 2;
    QSet<quint32> uniques;
    while (true)
    {
        for (const auto& fileEntry : filesList)
        {
            uniques.insert(fileEntry.hashFilename % tableSize);
        }
    ...
    QVector<QVector<FileEntry>> buckets(tableSize);
    for (const auto& fileEntry : filesList)
    {
        buckets[fileEntry.hashFilename % tableSize].push_back(fileEntry);
    }

    FileStream tocFile = FileStream(path, FileMode::Create, FileAccess::WriteOnly);
    tocFile.WriteUInt32(TOCTag);
    tocFile.WriteUInt32(0);
    tocFile.WriteUInt32(tableSize);
    ...
    for (const auto& bucket : qAsConst(buckets))
    {
        ...
        {
            tocFile.WriteUInt32(entryPos - tablePos);
            tocFile.WriteUInt32(bucket.size());
            for (const auto& fileEntry : bucket)
            {
                entryPos += ((28 + (fileEntry.path.length() + 1) + 3) / 4) * 4; // align to 4;
            }
        }
        tablePos += 8;
    }
    ...
    for (const auto& bucket : qAsConst(buckets))
    {
        for (const auto& fileEntry : bucket)
        {
            ...
            int blockSize = ((28 + (fileEntry.path.length() + 1) + 3) / 4) * 4; // align to 4
            tocFile.WriteUInt16((ushort)blockSize);
            tocFile.WriteUInt16(0);
            tocFile.WriteUInt32(fileEntry.size);
            tocFile.WriteZeros(20);
            tocFile.WriteStringASCIINull(fileEntry.path);
