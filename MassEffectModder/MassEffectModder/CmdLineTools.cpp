...
bool CmdLineTools::convertGameImage(MeType gameId, QString &inputFile, QString &outputFile, bool markToConvert)
{
    ...
    TreeScan::loadTexturesMap(gameId, resources, textures);
    bool status = convertGameTexture(inputFile, outputFile, textures, markToConvert);
...
bool CmdLineTools::CheckGameData(MeType gameId, bool ipc)
{
    ...
    QString errors;
    QStringList modList = QStringList();
    auto resources = new Resources();
    resources->loadMD5Tables();
                                               ...
                                               QString &textureName, QString &tfcName, QByteArray &guid)
{
    ...
    for (int n = 0; n < list->count(); n++)
    {
        ...
        {
            memcpy(origGuid, texture->properties->getProperty("TFCFileGuid").valueStruct.ptr(), 16);
            archiveFile = DirName(g_GameData->GamePath() + nodeTexture.path) + "/" + tfcName + ".tfc";
            texture->properties->setNameValue("TextureFileCacheName", tfcName);
        ...
        for (int m = 0; m < image->getMipMaps().count(); m++)
        {
                ...
                mipmap.storageType == Texture::StorageTypes::extUnc)
            {
                if (arcTexture == nullptr ||
                    memcmp(arcTexture->properties->getProperty("TFCFileGuid").valueStruct.ptr(),
                    texture->properties->getProperty("TFCFileGuid").valueStruct.ptr(), 16) != 0)
                {
                    triggerCacheArc = true;
                    Texture::TextureMipMap oldMipmap = texture->getMipmap(mipmap.width, mipmap.height);
                    if (memcmp(origGuid, texture->properties->getProperty("TFCFileGuid").valueStruct.ptr(), 16) != 0 &&
...
bool CmdLineTools::CheckTextures(MeType gameId, bool ipc)
{
    ...
    for (int i = 0; i < g_GameData->packageFiles.count(); i++)
    {
        ...
        for (int e = 0; e < package.exportsTable.count(); e++)
        {
            ...
            {
                Texture *texture = new Texture(package, e, package.getExportData(e));
                if (texture == nullptr)
                {
                    if (ipc)
                    {
